...
#Analyzing mean and median value for Tier1 categories
grouped_df = df.groupby('Tier_1')['price'].median().reset_index()
plt.figure(figsize=(20,15))
snp.barplot(y=grouped_df['price'], x=grouped_df["Tier_1"])
plt.title("Tier1 vs Price")
plt.xlabel("Tier1 category")
plt.show()
...
plt.figure(figsize=(20,15))
snp.barplot(x='brand_name', y='price', data=df.groupby('brand_name').price.mean().nlargest(10).reset_index())
plt.title('Brand_name vs Price')
plt.xticks(rotation=70)
plt.show()

# Rozkład cen produktów w zależności od jakośći podanej przez sprzedającego
df = df[df['price']<250]
my_plot = []
...
#Analyzing mean and median value for Tier1 categories
plt.figure(figsize=(20,15))
snp.barplot(x='Category', y='Mean price', data=df[["price","Tier1"]].groupby(by='Tier1').mean().values)
plt.show()
